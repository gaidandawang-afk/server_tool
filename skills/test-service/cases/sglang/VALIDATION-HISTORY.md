...
This preserves the former index verbatim. Read only for an explicit historical comparison; it does not validate a new source HEAD.
...
| Recoverable exception and whole-DP scale-down | `fault_exception_pause_scale_down.sh` | `fault-exception-pause-scale-down` | Non-overlap and overlap each passed 69/69 with 8-request pre-fault and 6-request post-scale-down concurrent gates (`nonoverlap-concurrent-scale-down-r2`, `overlap-concurrent-scale-down-r2`) | PASS ON `ff8e48cfe8` |
