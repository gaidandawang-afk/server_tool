...
sg_capture_interrupted_stream_contract \
  ...
  "$run_dir/pause-inflight-contract.json" 70 pause_rank1_after_kill
stream_pid=""

st_http_json POST "http://127.0.0.1:${port}/generate" \
  "$request_dp0" "$run_dir/paused-dp0.json" 503 paused_blocks_generate 90
...
st_http_json POST "http://127.0.0.1:${port}/generate" \
  "$request_dp0" "$run_dir/post-retry-dp0.json" 200 post_retry_dp0 90
sg_assert_output_ids_equal \
  "$run_dir/baseline-dp0.json" "$run_dir/post-retry-dp0.json" \
  "$run_dir/post-retry-precision.json" post_retry_dp0 10
