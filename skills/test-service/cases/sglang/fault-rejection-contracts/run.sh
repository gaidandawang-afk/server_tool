...
sg_assert_ft_failure_message \
  ...
  "Input tag 'recover'" recover_before_disabled_reason
...
sg_assert_ft_failure_message \
  ...
  "Input tag 'recover'" recover_before_rejoin_reason
...
st_http_json POST "http://127.0.0.1:${port}/generate" \
  "$run_dir/request-dp1.json" "$run_dir/dead-dp1-response.json" 503 \
  dead_dp1_rejected 60
sg_assert_inactive_route_error \
  "$run_dir/dead-dp1-response.json" 1 dead_dp1_inactive_error
st_http_json POST "http://127.0.0.1:${port}/generate" \
