...
| --- | --- | --- | --- | --- |
| Kill one scheduler, continue on three survivors | `fault_kill_continue_status_only.sh` | `fault-kill-continue-status-only` | continue, dead routing, survivor precision | Implemented; full corrected contract passed once on clean GPUs at `edfdb260…`; exact repetitions blocked by foreign GPU processes, so not yet Verified; see [validation record](VALIDATION-2026-07-29.md) |
| Kill one scheduler, pause and retry | `fault_kill_pause_retry.sh` | `fault-kill-pause-retry` | pause barrier, retry, survivor precision | Implemented; full corrected contract passed once on clean GPUs at `edfdb260…`; exact repetitions blocked by foreign GPU processes, so not yet Verified; see [validation record](VALIDATION-2026-07-29.md) |
...
| Kill one member of a TP sibling group | `fault_tpgt1_sibling_ep_retention.sh` | `fault-tpgt1-sibling-ep-retention` | TP>1 sibling retention and EP layout | Indexed |
