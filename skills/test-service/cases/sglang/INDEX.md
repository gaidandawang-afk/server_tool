...
| Kill one idle scheduler and commit whole-DP scale-down | `fault_kill_pause_scale_down.sh` | `fault-kill-pause-scale-down` | historical run: 503, no central pause, forced EPLB, dead target and three survivor precisions passed, 30/30 (`20260810-r1`); current contract preserves healthy idle survivors because no forward exception occurred | HISTORICAL PASS; CURRENT PENDING |
