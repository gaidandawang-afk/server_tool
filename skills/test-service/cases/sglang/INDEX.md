...
contracts target `codex/ft-self-pause-whole-dp` and the architecture defined by
`SELF_PAUSE_WHOLE_DP_FT.md`. They must be validated against the exact selected source HEAD.

**Validation state:** pending on `codex/ft-self-pause-whole-dp`. No active contract in this
...
- `scale_down` installs the explicit survivor topology, forces EPLB, shuts down every
  Scheduler member of each target DP, and resumes only after the route and shutdown barriers.
- Rejoin is always a complete native `nnodes` process-group lifecycle. ProcessUp alone remains
  `dead`; successful recovery forward/EPLB becomes provisional `disabled`; explicit recover
  commits the cohort and changes it to `healthy`.
...
| Recoverable exception and retry | `fault_exception_pause_retry.sh` | `fault-exception-pause-retry` | UNHEALTHY, maskless retry reset, full ACK, no EPLB | PENDING |
| Retry after a committed 4-to-3 scale-down | `fault_kill_scale_down_exception_retry.sh` | `fault-kill-scale-down-exception-retry` | local survivor exception, ACK `[0,2,3]`, no EPLB or re-expansion | PENDING |
...
| Scale down and rejoin a logical node with pause | `fault_kill_pause_scale_down_then_rejoin.sh` | `fault-kill-pause-scale-down-then-rejoin` | DEAD -> provisional DISABLED -> explicit recover -> HEALTHY | PENDING |
| Recoverable exception with continue/discard | `fault_exception_continue_discard_resume.sh` | `fault-exception-continue-discard-resume` | transient UNHEALTHY, next native report clears it, no retry reset | PENDING |
