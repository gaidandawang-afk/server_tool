...
| Kill during a stream with continue | `fault_kill_continue_inflight.sh` | `fault-kill-continue-inflight` | interrupted stream, continue, survivor precision | Implemented; runtime validation pending |
| Kill during a stream with pause/retry | `fault_kill_pause_inflight_retry.sh` | `fault-kill-pause-inflight-retry` | interrupted stream, pause block, retry | Implemented; runtime validation pending |
