...
| --- | --- | --- | --- | --- |
| Kill one scheduler, continue on three survivors | `fault_kill_continue_status_only.sh` | `fault-kill-continue-status-only` | continue, dead routing, survivor precision | Implemented; corrected environment produced one runtime failure and one pass on `edfdb260…`, so not yet Verified; see [validation record](VALIDATION-2026-07-29.md) |
| Kill one scheduler, pause and retry | `fault_kill_pause_retry.sh` | `fault-kill-pause-retry` | pause barrier, retry, survivor precision | Implemented; harness mismatches corrected, but Mooncake copy failures prevented a valid pass on `edfdb260…`; see [validation record](VALIDATION-2026-07-29.md) |
